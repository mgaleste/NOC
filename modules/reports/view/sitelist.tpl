			...
			<?php
				...
				if($num_rows>0){	
					for($i=0;$i<$num_rows;$i++){
						...
						$ids 				= mysql_result($result, $i, "sitestatusid");
...
$fp = fopen('pdf_reports/sitehistory_'.$datetime.'.xls', "wb");

chmod('pdf_reports/sitehistory_'.$datetime.'.xls',0777);
fwrite($fp, $excel);
fclose($fp);

header('Content-Description: File Transfer');
	header('Content-Type: application/octet-stream');
	header('Content-Disposition: inline; filename='.basename('pdf_reports/sitehistory_'.$datetime.'.xls'));
	header('Content-Transfer-Encoding: binary');
	header('Expires: 0');
	header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
	header('Pragma: public');
	header('Content-Length: ' . filesize('pdf_reports/sitehistory_'.$datetime.'.xls'));
	ob_clean();
	flush();
	readfile('pdf_reports/sitehistory_'.$datetime.'.xls');
	exit;
