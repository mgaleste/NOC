...
$paginate_arr['paginatequery'] 	=	"SELECT m.moduleCode as id, m.moduleCaption as modulecaption, m.moduleName as modulename, m.`type` as `type`
, m.imagePath as imagepath FROM modules m LEFT JOIN group_modules gm ON gm.moduleCode=m.moduleCode
WHERE m.stat='active' AND m.remarks!='' AND gm.groupCode='$currentGroupId' 
GROUP BY m.moduleCode ";
$paginate_arr['query'] 			=	"ORDER BY m.menuorder";
