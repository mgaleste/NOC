			...
			<table width="100%" cellpadding="0" cellspacing="0" id="myTable" class="tablesorter">					
				<thead>		
					<tr>
						<th colspan="5">
							<table cellpadding="0" cellspacing="0" border="0" width="100%">
								<tr valign="middle">						
									<td class="verdana10"><a href="index.php?mod=<?=$mod?>&type=<?=$type?>&task=create" class="sub_link"><img src="<?=MODIMAGES;?>create.gif" border="0" alt=""> Create New Ticket</a></td>
									<td class="verdana10"><?if($groupCode!='customer'){?><input style="border:none" type="image" src="<?=MODIMAGES;?>delete.gif" name="submit_delete" width="16" height="16"><button name="submit_delete" class="delbut">Delete</button>[<input name="hidden_selected" class="verdana10 input_hidden" type="text" id="removeChecked" value="0">]<button name="submit_delete"  class="delbut">Ticket(s)</button><?}?></td>
					...
					<tr valign="middle">
						...
						<th width="20%" class="table_line1_left">Ticket Number</th>
						<th width="20%" class="table_line1_left">Problem</th>
						<th width="15%" class="table_line1_left">Description</th>
			...
			<?php
				...
				if($num_rows>0){
					...
					for($i=0;$i<$num_rows;$i++){
						...
						$link				= ($groupCode!='customer')? "index.php?mod=$mod&type=$type&task=view&sid=$ids" : "";
                                                $linkStat			= ($groupCode!='customer')? "index.php?mod=$mod&type=$type&task=changestatus&sid=$ids" : "";
							...
							echo '	<td align="center" class="table_line2_"><input class="ibox" value="'.$ids.'" name="delAnn[]" onclick="checkAllFields(2);" type="checkbox"></td>
									<td class="table_line2_left" id="'.$link.'" onClick="viewRecord(event,this.id);">'.$ids.'&nbsp;</td>
                                                                        <td class="table_line2_left">'.stripslashes(html_entity_decode($problem)).'&nbsp;</td>
									<td class="table_line2_left">'.stripslashes(html_entity_decode($description)).'&nbsp;</td>';

                                                               if($customerStatus=='Done'){
                                                                        echo '<td class="table_line2_left_right" align="center"><b style="color:#'.$statusColor.'">'.stripslashes(html_entity_decode($customerStatus)).'</b>&nbsp;</td>';
								}else{
