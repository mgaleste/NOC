...
$.extend(InlineEditor.prototype, {
	...
	connectOpeningEvents: function() {
		...
		this.dom
			...
			.bind('mouseenter.editInPlace', function(anEvent){ that.openEditor(anEvent); });
	},
	...
	createEditorElement: function() {
		...
		/*editor = $('<input type="text" ' + this.inputNameAndClass() 
			...
			editor = $('<input type="text" ' + this.inputNameAndClass2() 
				+ ' size="' + this.settings.text_size  + '"/>');
	...
	inputNameAndClass2: function() {
		return ' name="inplace_value" class="textfield" ';
	},
	...
	connectClosingEventsToEditor: function() {
		...
		form.find(".textfield").focus().select();
		...
		if ( ! this.settings.show_buttons) {
			...
			if ("save" === this.settings.on_blur){
				...
				form.find(".textfield").blur(saveEditorAction);
			}else{
				form.find(".inplace_field").blur(cancelEditorAction);
				form.find(".textfield").blur(cancelEditorAction);
			}
